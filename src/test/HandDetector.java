...
public class HandDetector
{
  ...
  public void update(BufferedImage bufImage)
 ...
 */
  {
	  IplImage im = toIplImage(bufImage);
    ...
    cvInRangeS(hsvImg, cvScalar(0, 50, 50, 0),
                       cvScalar(43, 255, 255, 0), imgThreshed);
  ...
  */
  ...
  {
    int yOffset = (int)cogPt.y - (int)tipPt.y;    // make y positive up screen
    int xOffset = (int)tipPt.x - (int)cogPt.x;
  ...
  // draw information about the finger tips and the hand COG
  {
    ...
    for (int i=0; i < fingerTips.size(); i++) {
      ...
      if (namedFingers.get(i) == FingerName.UNKNOWN) {
        ...
        g2d.drawOval((int)(pt.x-8), (int)(pt.y-8), 16, 16);
        g2d.drawString("" + i, (int)pt.x, (int)(pt.y-10));   // label it with a digit
      }
      else {   // draw yellow line to the named finger tip from COG
        g2d.setPaint(Color.YELLOW);
        g2d.drawLine((int)cogPt.x, (int)cogPt.y, (int)pt.x, (int)pt.y);

        g2d.setPaint(Color.GREEN);   // named finger tip is green
        g2d.drawOval((int)(pt.x-8), (int)(pt.y-8), 16, 16);
        g2d.drawString(namedFingers.get(i).toString().toLowerCase(), (int)(pt.x), (int)(pt.y-10));
      }
    }

    // draw COG
    g2d.setPaint(Color.GREEN);
    g2d.fillOval((int)(cogPt.x-8), (int)(cogPt.y-8), 16, 16);
  }
